export default defineEventHandler(async (event) => {
  ...
  const [existing]: any = await db.query(
    "SELECT ID FROM tbl_posts WHERE post_title = ? OR post_name = ?",
    [title, slug],
  );
  if (existing.length > 0) {
    throw createError({
      statusCode: 400,
      statusMessage: "Judul atau Slug berita ini sudah digunakan",
    });
  }
  ...
  try {
    ...
    if (categories.length > 0) {
      for (const catId of categories) {
        if (catId) {
          await db.query(
            "INSERT INTO tbl_term_relationships (object_id, term_taxonomy_id) VALUES (?, ?)",
            [postId, catId],
          );
        }
      }
    ...
    if (tags.length > 0) {
      for (const tagName of tags) {
        ...
        // Cari atau buat term
        let [termRows]: any = await db.query(
          "SELECT term_id FROM tbl_terms WHERE slug = ?",
          [tagSlug],
        );
        let termId;

        if (termRows.length === 0) {
          const [newTerm]: any = await db.query(
            "INSERT INTO tbl_terms (name, slug) VALUES (?, ?)",
            [tagName, tagSlug],
          );
        ...
        let [taxRows]: any = await db.query(
          'SELECT term_taxonomy_id FROM tbl_term_taxonomy WHERE term_id = ? AND taxonomy = "post_tag"',
          [termId],
        );
        let termTaxonomyId;

        if (taxRows.length === 0) {
          const [newTax]: any = await db.query(
            'INSERT INTO tbl_term_taxonomy (term_id, taxonomy, description) VALUES (?, "post_tag", "")',
            [termId],
          );
        ...
        // Simpan relasi
        await db.query(
          "INSERT IGNORE INTO tbl_term_relationships (object_id, term_taxonomy_id) VALUES (?, ?)",
          [postId, termTaxonomyId],
        );

        // Update hitungan (count)
        await db.query(
          "UPDATE tbl_term_taxonomy SET count = count + 1 WHERE term_taxonomy_id = ?",
          [termTaxonomyId],
        );
      }
    }
    // 5. Simpan Gambar pendukung ke tbl_postmeta
    const postImages = getFiles("post_images");
    if (postImages.length > 0) {
      for (const file of postImages) {
        const fileData = await saveFile(file, "berita");
        if (fileData?.url) {
          await db.query(
            `INSERT INTO tbl_postmeta (post_id, meta_key, meta_value) VALUES (?, 'post_image', ?)`,
            [postId, fileData.url],
          );
        }
      }
