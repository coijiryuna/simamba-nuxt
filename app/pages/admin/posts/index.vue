...
const syncSimba = async () => {
  ...
  try {
    ...
    Swal.fire({
      ...
      text: `${response.syncedCount} berita baru berhasil disinkronkan dari SIMBA.`,
      confirmButtonColor: '#047857',
    })
  } catch (e) {
    Swal.fire('Gagal!', e.data?.message || 'Gagal menyinkronkan berita dari SIMBA.', 'error')
  } finally {
    syncing.value = false
  }
}

const { data: postsData, pending, refresh } = await useFetch('/api/v1/posts', {
  query: computed(() => ({
    search: search.value,
...
const counts = computed(() => {
  const data = postsData.value?.data || []
  return {
    all: data.length,
    publish: data.filter(p => p.post_status === 'publish').length,
    draft: data.filter(p => p.post_status === 'draft').length,
  }
})
</script>
...
<template>
  <div>
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-slate-800 mb-1 tracking-tight italic">Manajemen Konten</h1>
    ...
    <div
      ...
      <div class="flex items-center gap-2">
        ...
        <button @click="refresh" class="p-2 bg-slate-100 hover:bg-slate-200 rounded-sm text-slate-500 transition-all">
    ...
    <div class="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden relative min-h-[400px]">
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr v-if="postsData?.data?.length === 0">
            <td colspan="5" class="px-6 py-20 text-center">
              <div class="flex flex-col items-center gap-2">
                <FileText class="w-12 h-12 text-slate-200 mb-2" />
                <p class="text-sm font-bold text-slate-400 uppercase tracking-widest">Tidak ada konten ditemukan</p>
                <p class="text-[10px] text-slate-300 italic">Coba ubah filter atau kata kunci pencarian Anda.</p>
              </div>
            </td>
          </tr>
        </tbody>
