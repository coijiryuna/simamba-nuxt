...
<template>
  <div>
    <div class="flex items-center gap-4 mb-8">
      <NuxtLink to="/admin/posts" class="p-2 hover:bg-white rounded-sm border border-slate-200 text-slate-500 transition-all">
    ...
    <form @submit.prevent="submit" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2 space-y-6">
        <div class="bg-white p-6 border border-slate-200 rounded-sm shadow-sm">
          <div class="mb-6">
            <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Judul Berita</label>
            <input 
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden">
          ...
          <div class="p-4 space-y-4">
            <div>
              <label class="block text-xs font-bold text-slate-400 mb-2 italic tracking-tighter">Status Saat Ini: <span class="text-emerald-700 font-black uppercase">{{ form.post_status }}</span></label>
              <select v-model="form.post_status" class="w-full bg-slate-50 border border-slate-200 rounded-sm py-2 px-3 text-sm outline-none focus:border-emerald-600">
        ...
        <div class="bg-white border border-slate-200 rounded-sm shadow-sm">
          <div class="px-4 py-3 bg-slate-50 border-b border-slate-200 text-xs font-black uppercase tracking-widest text-slate-800">Kategori</div>
          <div class="p-4 h-48 overflow-y-auto custom-scrollbar">
            <div v-for="cat in categories" :key="cat.id" class="flex items-center gap-2 mb-2">
              <input type="checkbox" :id="`cat-${cat.id}`" v-model="form.categories" :value="cat.id" class="w-4 h-4 rounded-sm border-slate-300 text-emerald-600 focus:ring-emerald-500" />
        ...
        <!-- Tags Box -->
        <div class="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden">
          <div class="px-4 py-3 bg-slate-50 border-b border-slate-200 text-xs font-black uppercase tracking-widest text-slate-800">Tags</div>
          <div class="p-4">
            <input 
              v-model="form.tags"
              type="text" 
        ...
        <!-- Image Box -->
        <div class="bg-white border border-slate-200 rounded-sm shadow-sm overflow-hidden">
          <div class="px-4 py-3 bg-slate-50 border-b border-slate-200 text-xs font-black uppercase tracking-widest text-slate-800">Gambar Unggulan</div>
          <div class="p-4">
            <div v-if="imagePreview" class="relative mb-4 group">
              ...
              <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                ...
                <button class="bg-white text-slate-900 px-4 py-2 rounded-sm text-[10px] font-black uppercase tracking-widest">Ganti Gambar</button>
              </div>
